...
opt = {
   DATA_ROOT = '/home/panziqi/project/20190304_action_recognition/Im2Flow/video_dataset/ucf101/merged/',         -- path to images (should have subfolders 'train', 'val', etc)
   name = 'flow_ucf101_train',              -- name of the experiment, should generally be passed on the command line
   batchSize = 32,         -- # images in batch
   ...
   niter = 30,            -- #  of iter from starting learning rate
   ...
   gpu = 4,                -- gpu = 0 is CPU mode. gpu=X is GPU mode on GPU X
   ...
   decay_epoch_freq = 10,      -- decay the learning rate by half  every decay_epoch_freq
   save_latest_freq = 2000,     -- save the latest model every latest_freq sgd iterations (overwrites the previous latest model)
   print_freq = 20,             -- print the debug information every print_freq iterations
   ...
   lambda_L2 = 50,            -- weight on L2 term in objective
   lambda_ContentLoss = 1,   -- weight on Content Loss term in objective
}
